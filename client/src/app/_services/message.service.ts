...
export class MessageService {
  ...
  createHubConnection(user: User, recipientUsername: string){
    ...
    this.hubConnection.on('UpdatedGroup', (group: Group) => {
      if (group.connection.some(x => x.username === recipientUsername)) {
        this.messageThread$.pipe(take(1)).subscribe(messages => {
  ...
  deleteMessage(photoId: number) {
    return this.http.delete(this.baseUrl + 'messages/' + photoId);
  }

  unsendMessage(photoId: number) {
    return this.http.delete(this.baseUrl + 'messages/unsend/' + photoId);
  }
